...
class Graph
{
	private:
		...
		//Gibt den richtigen Folgepointer zurueck:
		// -> right, wenn hoeher; left, wenn niedriger;
		Node<Nodedata, Value>* determineNextPosition(Node<Nodedata, Value>* node, Node<Nodedata, Value>* rootnode) {
			if (node->getValue() > rootnode->getValue()) {
				...
				//Gleicher Schluesselwert und Objektadresse. Folglich soll kein Eintrag stattfinden.
				return NULL;
			}
			else {
		...
		bool determineNodePosition(Node<Nodedata, Value>* node, Node<Nodedata, Value>* rootnode) {
			...
			//Wenn der naechste Platz nicht frei ist, such weiter.
	...
	public:
		...
		//Fuegt ein Node bei return=true hinzu. True = erfolgreich, False = identischer Eintrag.
		...
		//Loescht ein Node und gibt "true" zurueck, wenn erfolgreich
		bool deleteNode(Nodedata* node, Value value) {
			if(_rootnode != NULL){
				...
				//true, wenn zu loeschender Node nicht gefunden wurde.
				bool finish = false;
				while(!finish){
					//Wenn Node gefunden
					if (deletenode->getValue() == value && deletenode->getData() == node) {
						//Node welcher den zu loeschenden Node ersetzt
						Node<Nodedata, Value>* remembernode = NULL;
						//Vorgaenger von remembernode
						...
						if(deletenode->getRightNode() != NULL){
							...
							//Gehe nun so weit wie moeglich nach Links.
						...
						//Wenn es einen Vorgaenger vom zu loeschenden Node gibt,
						if(lastnode != NULL){
							//passe die Edges an.
